...
int x = 0, par, used[100], free_mem, ifrag = 0, pifrag[100], efrag = 0, pos[100], p[100],  pr[100], ap[100], loc[100], q[100], front = 0, rear = 0;
void wf_sort(), mem_alloc(), mem_dealloc(), display();
...
void mem_alloc()
{
  ...
  for(j=0;j<par;j++)
  {
      ...
      else
      {
        if(used[j] == -1)
        {
          cout<<"Process "<<x+1<<" allocated to Partition "<<loc[j]<<"\n";
          pos[x] = loc[j];
